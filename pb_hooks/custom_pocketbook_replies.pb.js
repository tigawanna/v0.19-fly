...
(SELECT COUNT(*) FROM pocketbook_posts WHERE parent = pp.id AND depth = ({:depth} + 1)  ) replies,
IFNULL((SELECT  id FROM pocketbook_posts WHERE pp.user = {:user} AND parent = pp.id AND depth = ({:depth}  + 1) )
,'virgin') myreply
...
// http://127.0.0.1:8090/custom_pocketbook_post_replies?depth=1&created=2023-11-21T20:44:29+03:00Z&limit=2&profile=general&parent=x7x2zg4b5vg928a
// parameters required
// id: c.queryParam("id") : id of the post we're looking up
// user: c.queryParam("user") : id of the currently logged in user
// parent: c.queryParam("parent") : id of the parent post (post you're replying to)
